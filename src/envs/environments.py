...
class instance():
    ...
    def updateToggles(self):
        for k, v in self.toggles.items():
            ...
            if v[0] == 'button_red':
                # colorstate = self.bullet_client.getVisualShapeData(v[1])[0][7]
                # colorstate = list(colorstate)
                if jointstate < -0.05:
            ...
            if v[0] == 'button_black':
                ...
                if jointstate < -0.05:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[0,0,0,1])
                else:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[1, 1, 1, 1])
            if v[0] == 'dial':

                if dial_to_0_1_range(jointstate) < 0.125:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[1,1,1,1])
                elif dial_to_0_1_range(jointstate) < 0.25:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[1,1,0,1])
                elif dial_to_0_1_range(jointstate) < 0.375:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[0,1,0,1])
                elif dial_to_0_1_range(jointstate) < 0.5:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[0,1,1,1])
                elif dial_to_0_1_range(jointstate) < 0.625:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[0,0,1,1])
                elif dial_to_0_1_range(jointstate) < 0.75:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[1,0,1,1])
                elif dial_to_0_1_range(jointstate) < 0.875:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[0,0,0,1])
                else:
                    self.bullet_client.changeVisualShape(v[1], -1, rgbaColor=[1,0,0,1])
